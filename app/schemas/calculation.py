...
class CalculationType(str, Enum):
    ...
    DIVISION = "division"
    EXPONENTIATION = "exponentiation"  # ✅ new feature


class CalculationBase(BaseModel):
    type: str = Field(..., description="Type of calculation", example="addition")
    ...
    def validate_type(cls, v):
        ...
        if v_lower not in allowed:
            # ✅ make error contain "one of" like tests expect
            raise ValueError(f"Type must be one of: {', '.join(sorted(allowed))}")
    ...
    def check_inputs_is_list(cls, v):
        if not isinstance(v, list):
            # ✅ EXACT string your test expects
    ...
    def validate_inputs(self) -> "CalculationBase":
        ...
        if self.type == CalculationType.DIVISION.value:
            if any(x == 0 for x in self.inputs[1:]):
                raise ValueError("Cannot divide by zero")

        # ✅ exponentiation rule: exactly 2 inputs
        if self.type == CalculationType.EXPONENTIATION.value:
            if len(self.inputs) != 2:
